...
class DB_manager:
    ...
    def _add_longitudinal_index(self, df):
        df[SESSIONS_COUNTER] = df.groupby([CAGE, MOUSE_NAME, FOV])[EXPERIMENT_DATE].rank(method='dense', ascending = False).astype(int)
        return df
    ...
    # # run over each row in the db and change the value of a column
    # def add_strain_val(self, df):
    #     for row in range(len(df)):
    #         strain = pipe_utils.get_mouse_strain(df.loc[row,CAGE])
    #         self.db.loc[row, STRAIN] = strain
    #     self.db.to_csv(paths.DB_PATH, index=False, header=False)
...
class Experiment:
    ...
    def remove_spike_activty(self, df):
...
class FigGenerator:
    ...
    def _create_fig_spike_detection(self, experiment):
        from utils import spike_detection_from_db as sd
