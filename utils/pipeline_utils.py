...
def get_fov_name_windows(raw_path):
    path_col_split = raw_path.split('/')
    for word in path_col_split:
        if "FOV" in word:
            return word
...
def get_mouse_strain(cage): #returns mouse strain according to cage name
    lety = cage[0]
    if lety == 'W':
        return 'Wild Type'
    if lety == 'S':
...
def save_record_to_DB_queue(experiment_date, cage, mouse_name, seq, cell_type, strain, video_path):
    df = pd.read_csv(paths.DB_UPLOAD_QUEUE_PATH)
    row = [experiment_date, cage, mouse_name, seq, cell_type, strain, video_path]
    df.loc[len(df)] = row
