...
def normalize_thresh(tresh, trace):
    tresh = (tresh - trace.min()) / (trace.max() - trace.min())
    return tresh


def explore_threshold(trace, fr, detection_method, segments_num, hyper_param_lst, manual_thresh_lst):
    ...
    for i, segment in enumerate(segments):
        ...
        th = normalize_thresh(th, hp_segment)
        hp_segment = normalize_trace(hp_segment)
        if manual_thresh_lst[i] != 0.0:
            spikes = get_spikes_for_given_threshold(hp_segment, manual_thresh_lst[i])[0]
            th = manual_thresh_lst[i]
...
def get_exploration_figure(cage_name, mouse_name, pipeline_seq, cell, cell_type, trace, hp_trace, spike_time, ths, segments_size, spike_time_from_file):
    ...
    fig.add_scatter(name='cell '+str(cell) + " spikes", x=spike_time, y=spike_value, mode='markers', secondary_y=True,
                    marker=dict(size=3, color=spike_color), visible= True, row=1, col=1, showlegend=True),
    fig.add_scatter(name='cell '+str(cell) + " spikes in file", x=spike_time_from_file, y=file_spike_value, mode='markers', secondary_y=True,
                    marker=dict(size=3, color='purple'), visible = 'legendonly', row=1, col=1, showlegend=True)
...
def get_spikes_for_given_threshold(trace, threshold):
    spikes = signal.find_peaks(trace, height=threshold)
    return spikes
...
def sliders_display(good_cells, cell_type):
    ...
    counti = 1 #will be used to change the key of the sliders
    try:
        with col1:
        ...
        counti +=1
        with col1:
            cell = st.radio('Select cell: ', good_cells, key = 'cell_selector' + ' 2')
        with col2:
            detection_methods = ['Volpy', 'Median filter']
            detection_method = st.radio("Detection method:", detection_methods, key = str(cell) + ' 2', index = default_method)
        with col3:
            seg_num = 2 if cell_type == 'IN' else 1
            segments_num = st.select_slider('Select trace partitioning amount', options = np.arange(1,11), key = 'segments_slider_celll_' + str(cell) + ' 2', value = seg_num)
    ...
    for i, col in enumerate(st.columns(int(segments_num))):
        ...
        with col:
            # slider_initial_value = 0.5 if detection_methods == 'Volpy' else 4.5
            slider = st.select_slider(slider_text, options = slider_range, help = help_text, key =str(cell) + slider_key + str(i) + str(counti), value = slider_initial_value)
            thresh_slider = st.slider('Manual threshold', min_value= 0.0, max_value=1.0, value=0.0, step= 1/ 100, key= str(cell) + str(i) + 'manual_thresh')

        parameters_sliders_lst.append(slider)
        thresh_sliders_lst.append(thresh_slider)
    return parameters_sliders_lst, detection_method, segments_num, cell, thresh_sliders_lst


def run_spike_detection(cage_name, mouse_name, pipeline_seq, good_cells, cell_type, fov, fr, df):
    parameters_sliders_lst, detection_method, segments_num, cell, manual_tresh_lst = sliders_display(good_cells, cell_type)
    trace = get_cell_trace(df, cell)
    spike_time_from_file = df[df[SPIKE_COL+str(cell)]==1].index 
    trace, hp_trace, spike_time, ths, segments_size = explore_threshold(trace, fr, detection_method, segments_num, parameters_sliders_lst, manual_tresh_lst)
    ...
    for c in [c1,c2,c4,c5]:
        with c:
            pass
    with c3:
        save_spike_train = st.button('Save spikes', key = "save_button_" + str(cell)+mouse_name+cage_name)
    if save_spike_train:
